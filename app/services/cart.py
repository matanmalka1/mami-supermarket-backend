"""Cart service for customer cart operations."""
...
class CartService:
    ...
    def _assert_in_stock_anywhere(product: Product) -> None:
        total_available = db.session.scalar(
            select(func.coalesce(func.sum(Inventory.available_quantity), 0)).where(Inventory.product_id == product.id)
        )
    ...
    def _reload_cart(cart_id: UUID) -> Cart:
        ...
        cart = db.session.execute(stmt).scalar_one()
        return cart
